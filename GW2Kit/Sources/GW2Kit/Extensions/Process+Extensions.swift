...
public extension Process {
    ...
    private func makeStream(name: String, fileHandle: FileHandle?) -> AsyncStream<ProcessOutput> {
        ...
        return AsyncStream<ProcessOutput> { continuation in
            ...
            pipe.fileHandleForReading.readabilityHandler = { pipe in
                ...
                Logger.gw2Kit.info("\(line, privacy: .public)")
            ...
            errorPipe.fileHandleForReading.readabilityHandler = { pipe in
                ...
                guard !line.isEmpty else { return }
                Logger.gw2Kit.warning("\(line, privacy: .public)")
                fileHandle?.write(line: line)
            }
            ...
            terminationHandler = { (process: Process) in
                do {
                    ...
                    Logger.gw2Kit.error("Error while clearing data: \(error)")
                }
    ...
    private func logProcessInfo(name: String) {
        Logger.gw2Kit.info("Running process \(name)")

        if let arguments = arguments {
            Logger.gw2Kit.info("Arguments: `\(arguments.joined(separator: " "))`")
        }
        if let executableURL = executableURL {
            Logger.gw2Kit.info("Executable: `\(executableURL.path(percentEncoded: false))`")
        }
        if let directory = currentDirectoryURL {
            Logger.gw2Kit.info("Directory: `\(directory.path(percentEncoded: false))`")
        }
        if let environment = environment {
            Logger.gw2Kit.info("Environment: \(environment)")
        }
    }
}

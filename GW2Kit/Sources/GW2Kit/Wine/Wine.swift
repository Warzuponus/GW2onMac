...
public class Wine {
    ...
    private static func applyNativeDyldLibraryPath(to wineEnv: inout [String: String]) {
        let nativePath = WineRuntimeInstaller.nativeDyldLibraryPath
        guard !nativePath.isEmpty else { return }

        if let existing = wineEnv["DYLD_LIBRARY_PATH"], !existing.isEmpty {
            wineEnv["DYLD_LIBRARY_PATH"] = nativePath + ":" + existing
        } else {
            wineEnv["DYLD_LIBRARY_PATH"] = nativePath
        }
    }
...
extension Wine {
    ...
    )[0].appending(path: "Logs").appending(path: AppPaths.bundleIdentifier)
...
extension Wine {
    ...
    /// Initialize a fresh prefix or update an existing one (mirrors setup-gw2-prefix.sh).
    public static func initializePrefix(bottle: Bottle) async throws {
        let systemReg = bottle.url.appending(path: "system.reg")
        if FileManager.default.fileExists(atPath: systemReg.path) {
            _ = try await runWine(["wineboot", "-u"], bottle: bottle)
        } else {
            _ = try await runWine(["wineboot", "--init"], bottle: bottle)
        }
    }
}
